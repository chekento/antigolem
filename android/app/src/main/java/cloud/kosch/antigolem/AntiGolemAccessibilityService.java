        ...
        takeScreenshot(Display.DEFAULT_DISPLAY,getMainExecutor(),new TakeScreenshotCallback(){
            @Override public void onSuccess(ScreenshotResult screenshot){
                ...
                try{
                    ...
                    rec.process(img).addOnSuccessListener(txt->{String captured=txt.getText()==null?"":txt.getText().trim();if(overlayRoot!=null)overlayRoot.setVisibility(View.VISIBLE);if(captured.isEmpty())Toast.makeText(this,"OCR found no readable text in the selected area.",Toast.LENGTH_LONG).show();else{storeCaptured(captured);showQuickResult(captured,"Circle OCR");}rec.close();if(finalCrop!=null&&!finalCrop.isRecycled())finalCrop.recycle();if(finalCopy!=null&&!finalCopy.isRecycled())finalCopy.recycle();}).addOnFailureListener(err->{if(overlayRoot!=null)overlayRoot.setVisibility(View.VISIBLE);Toast.makeText(this,"OCR failed: "+err.getMessage(),Toast.LENGTH_LONG).show();rec.close();if(finalCrop!=null&&!finalCrop.isRecycled())finalCrop.recycle();if(finalCopy!=null&&!finalCopy.isRecycled())finalCopy.recycle();});
                }catch(Exception ex){if(overlayRoot!=null)overlayRoot.setVisibility(View.VISIBLE);Toast.makeText(this,"Circle OCR failed: "+ex.getMessage(),Toast.LENGTH_LONG).show();if(crop!=null&&!crop.isRecycled())crop.recycle();if(copy!=null&&!copy.isRecycled())copy.recycle();}finally{hb.close();}
            }
